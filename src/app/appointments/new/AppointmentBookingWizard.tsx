...
export default function AppointmentBookingWizard({
  ...
  // Editable Doctor Fee for receptionist (allows manual fee override)
  const [doctorFeeInput, setDoctorFeeInput] = useState<string>("");

  // Sync doctorFeeInput whenever selected doctor or appointment type changes
  useEffect(() => {
    if (serviceCategory === "OPD") {
      setDoctorFeeInput(String(doctorFees.active));
    }
  }, [doctorFees.active, serviceCategory]);
  ...
  const handleBookAppointment = async (e: React.FormEvent) => {
    ...
    const manualDocFee = doctorFeeInput.trim() !== "" && !isNaN(Number(doctorFeeInput)) ? Number(doctorFeeInput) : doctorFees.active;

    if (!isDiag && (isNaN(manualDocFee) || manualDocFee < 0)) {
      setSubmitError("Please enter a valid consultation fee amount.");
      return;
    }
    ...
    const effectiveFeeNum = isDiag ? Number(testFee) : manualDocFee;
    ...
    try {
      ...
      const effectiveFee = String(data.appointment.consultationFee ?? effectiveFeeNum);
      ...
      <form onSubmit={handleBookAppointment} className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 space-y-2">
            ...
            {/* MR Lookup Feedback */}
            {mrLookupStatus && (
              <div className={`text-xs p-2 rounded-lg flex items-center gap-2 ${mrLookupStatus.found
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`p-3 rounded-xl border text-left transition flex flex-col justify-between cursor-pointer ${isSelected
        ...
        {serviceCategory === "OPD" && (
          <>
              ...
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                ...
                {selectedDoctor && (
                  <span className="text-xs font-bold text-emerald-700">
                    Active Fee: PKR {Number(doctorFeeInput !== "" ? doctorFeeInput : doctorFees.active).toLocaleString()}
                  </span>
                )}
              </div>
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-3 text-xs text-teal-900">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-lg font-black text-emerald-800">
                        PKR {Number(doctorFeeInput !== "" ? doctorFeeInput : doctorFees.active).toLocaleString()}
                      </span>
                    </div>
                  </div>
            ...
            <div className="space-y-4 pt-2">
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  <button
                    ...
                    onClick={() => {
                      setAppointmentType("REGULAR");
                      setDoctorFeeInput(String(doctorFees.regular));
                    }}
                    className={`py-2.5 px-3 rounded-xl text-xs font-bold border transition flex flex-col items-center justify-center gap-0.5 cursor-pointer ${appointmentType === "REGULAR"
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setDoctorFeeInput(String(doctorFees.followUp));
                    }}
                    className={`py-2.5 px-3 rounded-xl text-xs font-bold border transition flex flex-col items-center justify-center gap-0.5 cursor-pointer ${appointmentType === "FOLLOW_UP"
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setDoctorFeeInput(String(doctorFees.emergency));
                    }}
                    className={`py-2.5 px-3 rounded-xl text-xs font-bold border transition flex flex-col items-center justify-center gap-0.5 cursor-pointer ${appointmentType === "EMERGENCY"
              ...
              {/* Editable Doctor Consultation Fee Input for Receptionist */}
              <div className="bg-emerald-50/70 border border-emerald-300 rounded-xl p-3.5 space-y-2">
                <div className="flex items-center justify-between">
                  <label className="text-xs font-bold text-slate-800 flex items-center gap-1.5">
                    <DollarSign className="w-4 h-4 text-emerald-600" />
                    <span>Doctor Consultation Fee (PKR) <span className="text-rose-500">*</span></span>
                  </label>
                ...
                <div className="relative">
                  <span className="absolute left-3 top-2.5 text-xs font-bold text-emerald-700 pointer-events-none">
                    PKR
                  </span>
                  <input
                    type="number"
                    ...
                    className="w-full text-base font-mono font-black pl-12 pr-3 py-2 border border-emerald-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 bg-white text-emerald-900"
                  />
                </div>
                <div className="flex items-center justify-between text-[11px] text-slate-500">
                  <span>Standard tier fee: PKR {Number(doctorFees.active).toLocaleString()} ({appointmentType.toLowerCase()})</span>
                  {doctorFeeInput !== String(doctorFees.active) && (
                    <button
                      type="button"
                      onClick={() => setDoctorFeeInput(String(doctorFees.active))}
                      className="text-teal-700 font-semibold hover:underline cursor-pointer"
                    >
                      Reset to standard (PKR {Number(doctorFees.active).toLocaleString()})
                    </button>
                  )}
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    <button
                      ...
                      className={`text-[11px] font-semibold px-2 py-0.5 rounded border transition ${appointmentDate === new Date().toISOString().split("T")[0]
                  ...
                  {(DIAGNOSTIC_PRESETS[serviceCategory] || []).map((preset) => {
                    ...
                    return (
                      <button
                        ...
                        className={`text-xs px-2.5 py-1 rounded-md font-semibold border transition flex items-center gap-1.5 cursor-pointer ${isPicked
