...
export default async function DoctorDashboardPage() {
  ...
  const [
    ...
    prisma.appointment.count({
      where: { ...whereDoctorToday, status: { in: ["WAITING", "SCHEDULED"] } },
    }),
    prisma.appointment.count({
      where: { ...whereDoctorToday, status: "IN_CONSULTATION" },
    }),
    prisma.appointment.count({
      where: { ...whereDoctorToday, status: "COMPLETED" },
    }),
    prisma.appointment.count({
      where: { ...whereDoctorToday, isEmergency: true },
    }),
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-white p-6 sm:p-8 rounded-xl border border-slate-200 shadow-xs">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            >
              ...
              <span>Open Patient Queue ({waitingCount})</span>
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-xs">
            ...
            <p className="text-2xl font-bold text-slate-900 mt-1">{todayTotal}</p>
          </div>
          ...
          <div className="bg-white p-4 rounded-xl border border-rose-200 shadow-xs bg-rose-50/20 col-span-2 sm:col-span-1">
            ...
            <p className="text-2xl font-bold text-rose-700 mt-1">{emergencyCount}</p>
          </div>
        </div>
                  ...
                  >
                    <div>
                      ...
                      <p className="text-slate-500 mt-0.5">
                        {apt.patient.mrNumber || apt.patient.patientNumber} • {apt.reason}
                      </p>
                    </div>
